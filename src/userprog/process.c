...
/* Methods for struct process and struct process_file. */
...
// TODO Init at once!!!!!!!!!!!!!!!! why? I don't know
struct process* process_init (void)
{
...
process_execute (const char *file_name) 
{
  ...
  // pass file_name with only program name to a thread.
  char* thread_name = (char*) malloc ((strlen(file_name)+1)*sizeof(char));

  // TODO Testing for multi-oom
...
start_process (void *file_name_)
{
  ...
  // init struct process and set relationships b/w parent.
  struct thread* cur = thread_current ();
...
{
  struct thread* cur = thread_current ();
  // TODO wait has not been called twice.
  // TODO There might be a problem!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  // TODO if the process loaded fail, return -1.
  if (!get_child_process (child_tid)->is_load) return -1;

  // TODO Below can be simplified... but annoying...
  struct list_elem* e;
  int flag = 0;
  struct process* p;
  for (e = list_begin (&(cur->child_processes));
  ...
  // We may be able to remove process here.
...
process_exit (void)
{
  ...
  // wait the parent up when exiting a process.
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  // TODO file_deny after opening file.
...
/* Add more arguments to identify what to put on stack. */
